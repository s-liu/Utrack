...
var ACTIVITIES = ['Writing Code', 'Eating Dinner', 'Playing Sports', 'Studying for Exams', 'Attending Lectures', 'Watching TV'];
...
_.extend(ActivityStoreModel.prototype, {
    ...
    getActivityTotalDuration: function (activityType) {
        var duration = 0;
        _.each(
            this.activityDataPoints,
            function (dataPoint) {
                if (dataPoint.activityType == activityType) {
                    duration += dataPoint.activityDurationInMinutes;
                }
            }
        );
        return duration;
...
var GraphModel = function () {
    ...
    this.listeners = [];
    this.availableGraphs = ['pie_chart', 'bar_chart'];
};
...
_.extend(GraphModel.prototype, {
    ...
    selectGraph: function (graphName) {
        if (this.availableGraphs.indexOf(graphName) !== -1 && this.currentSelectedGraph !== graphName) {
    ...
    reDraw: function() {
        _.each(
            this.listeners,
            function (listener_fn) {
                listener_fn(GRAPH_SELECTED_EVENT, new Date(), this.getNameOfCurrentlySelectedGraph());
            }, this
        );
    }
...
function generateFakeData(activityModel, numDataPointsToGenerate) {
    ...
    numDataPointsToGenerate = (!_.isNumber(numDataPointsToGenerate)
    || numDataPointsToGenerate < 0) ? 100 : numDataPointsToGenerate;
    _.times(
